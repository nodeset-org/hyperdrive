...
func (p *ExecutionClientManager) CheckStatus(cfg *config.HyperdriveConfig) *ClientManagerStatus {
	...
	if status.FallbackEnabled {
		...
		expectedChainID := cfg.GetChainID()
...
func IsSyncWithinThreshold(ec sharedtypes.IExecutionClient) (bool, time.Time, error) {
...
func GetEthClientLatestBlockTimestamp(ec sharedtypes.IExecutionClient) (uint64, error) {
	// Get latest block
