...
func createExternalBnSelectStep(wiz *wizard, currentStep int, totalSteps int) *choiceWizardStep {
	...
	clients := wiz.md.Config.ExternalBeaconConfig.BeaconNode.Options
	clientNames := []string{}
	...
	show := func(modal *choiceModalLayout) {
		...
		for i, option := range wiz.md.Config.ExternalBeaconConfig.BeaconNode.Options {
			if option.Value == wiz.md.Config.ExternalBeaconConfig.BeaconNode.Value {
				modal.focus(i)
				break
			}
		}
		...
		switch wiz.md.Config.ExternalBeaconConfig.BeaconNode.Value {
