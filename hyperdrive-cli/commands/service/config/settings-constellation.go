...
func (configPage *ConstellationConfigPage) handleLayoutChanged() {
	...
	if configPage.masterConfig.Constellation.Enabled.Value {
		...
		for _, item := range configPage.constellationItems {
			if item.parameter.GetCommon().ID == ids.ConstellationEnableID {
				continue
			}
			constellationItems = append(constellationItems, item)
		}
		configPage.layout.addFormItems(constellationItems)
