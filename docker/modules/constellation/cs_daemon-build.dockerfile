...
RUN if [ "$BUILDPLATFORM" = "linux/amd64" -a "$TARGETARCH" = "arm64" ]; then \
    ...
    cd /hyperdrive/modules/constellation/constellation-daemon && \
    GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -o /build/hyperdrive-constellation-daemon
# Copy the output
FROM scratch AS daemon
ARG TARGETOS TARGETARCH
COPY --from=builder /build/hyperdrive-constellation-daemon /hyperdrive-constellation-daemon
