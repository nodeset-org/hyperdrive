...
TOTAL_STEPS="9"
# The Hyperdrive user data path
HD_PATH="$HOME/.hyperdrive"
# The default Hyperdrive package version to download
PACKAGE_VERSION="latest"
# The default network to run Hyperdrive on
NETWORK="mainnet"
...
get_escalation_cmd() {
    if type sudo > /dev/null 2>&1; then
        ...
        fail "Please make sure a privilege escalation command such as \"sudo\" is installed and available before installing Hyperdrive."
    fi
}
...
if [ -d $HD_PATH ]; then
    ...
    if [ -f "$HD_PATH/user-settings.yml" ]; then
        progress 5 "Backing up configuration settings to user-settings-backup.yml..."
        { cp "$HD_PATH/user-settings.yml" "$HD_PATH/user-settings-backup.yml" || fail "Could not backup configuration settings."; } >&2
    fi
fi


# Create ~/.hyperdrive dir & files
progress 6 "Creating Hyperdrive directory structure..."
{ mkdir -p "$HD_PATH/modules" || fail "Could not create the Hyperdrive runtime directory."; } >&2
{ mkdir -p "$HD_PATH/runtime" || fail "Could not create the Hyperdrive runtime directory."; } >&2
{ mkdir -p "$DATA_PATH" || fail "Could not create the Hyperdrive data directory."; } >&2
{ mkdir -p "$HD_PATH/extra-scrape-jobs" || fail "Could not create the Prometheus extra scrape jobs directory."; } >&2


# Download and extract package files
progress 7 "Downloading Hyperdrive package files..."
...
# Copy package files
progress 8 "Copying package files to Hyperdrive user data directory..."
{ cp -r -n "$PACKAGE_FILES_PATH/override" "$HD_PATH" || rsync -r --ignore-existing "$PACKAGE_FILES_PATH/override" "$HD_PATH" || fail "Could not copy new override files to the Hyperdrive user data directory."; } >&2
{ cp -r "$PACKAGE_FILES_PATH/scripts" "$HD_PATH" || fail "Could not copy scripts folder to the Hyperdrive user data directory."; } >&2
{ cp -r "$PACKAGE_FILES_PATH/templates" "$HD_PATH" || fail "Could not copy templates folder to the Hyperdrive user data directory."; } >&2
{ cp "$PACKAGE_FILES_PATH/grafana-prometheus-datasource.yml" "$PACKAGE_FILES_PATH/prometheus.tmpl" "$HD_PATH" || fail "Could not copy base files to the Hyperdrive user data directory."; } >&2
{ find "$HD_PATH/scripts" -name "*.sh" -exec chmod +x {} \; 2>/dev/null || fail "Could not set executable permissions on package files."; } >&2
{ touch -a "$HD_PATH/.firstrun" || fail "Could not create the first-run flag file."; } >&2
