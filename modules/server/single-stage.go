...
import (
	...
	"github.com/nodeset-org/hyperdrive/hyperdrive-daemon/module-utils/services"
...
func RegisterSingleStageRoute[ContextType ISingleStageCallContext[DataType], DataType any](
	...
	logger *slog.Logger,
	serviceProvider services.IModuleServiceProvider,
) {
...
func RegisterSingleStagePost[ContextType ISingleStageCallContext[DataType], BodyType any, DataType any](
	...
	router.HandleFunc(fmt.Sprintf("/%s", functionName), func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			err = server.HandleInputError(logger, w, fmt.Errorf("error reading request body: %w", err))
			if err != nil {
				logger.Error("Error handling response", log.Err(err))
			}
			return
		...
		if err != nil {
			err = server.HandleInputError(logger, w, fmt.Errorf("error deserializing request body: %w", err))
			if err != nil {
				logger.Error("Error handling response", log.Err(err))
			}
			return
...
func runSingleStageRoute[DataType any](ctx ISingleStageCallContext[DataType], serviceProvider services.IModuleServiceProvider) (types.ResponseStatus, *types.ApiResponse[DataType], error) {
	// Get the services
