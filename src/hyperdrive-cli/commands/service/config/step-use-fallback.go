...
func createUseFallbackStep(wiz *wizard, currentStep int, totalSteps int) *choiceWizardStep {
	...
	show := func(modal *choiceModalLayout) {
		...
		if buttonIndex == 1 {
			...
			if (wiz.md.Config.Hyperdrive.ClientMode.Value == nmc_config.ClientMode_Local && wiz.md.Config.Hyperdrive.LocalBeaconConfig.BeaconNode.Value == nmc_config.BeaconNode_Prysm) ||
				(wiz.md.Config.Hyperdrive.ClientMode.Value == nmc_config.ClientMode_External && wiz.md.Config.Hyperdrive.ExternalBeaconConfig.BeaconNode.Value == nmc_config.BeaconNode_Prysm) {
	...
	back := func() {
		if wiz.md.Config.Hyperdrive.ClientMode.Value == nmc_config.ClientMode_Local {
